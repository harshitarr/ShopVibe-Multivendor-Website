...
// Safe fallback if address list is undefined
const addressList = useSelector(state => state.address.list) || [];
const addressLoading = useSelector(state => state.address.loading) || false;

console.log('OrderSummary - Address state:', { addressList, addressLoading, listLength: addressList.length });
...
const handlePlaceOrder = async (e) => {
    ...
    try {
        ...
        const orderData = {
            addressId: selectedAddress._id, // Fix: use _id instead of id
            items,
            paymentMethod
        }
    ...
    <div className='w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7'>
        ...
        <div className='my-4 py-4 border-y border-slate-200 text-slate-400'>
            ...
            {selectedAddress ? (
                ...
                <div>
                    {addressLoading ? (
                        <p className='text-slate-400 text-sm my-3'>Loading addresses...</p>
                        ...
                        <p className='text-slate-400 text-sm my-3'>No addresses found. Please add an address.</p>
                    )}
        ...
        <div className='pb-4 border-b border-slate-200'>
            <div className='flex justify-between'>
                ...
                <div className='flex flex-col gap-1 font-medium text-right'>
                    ...
                    <p><Protect plan={"plus"} fallback={`${currency}90`}>Free</Protect></p>
                    {coupon && <p>{`-${currency}${((coupon.discount || 0) / 100 * totalPrice).toFixed(2)}`}</p>}
                </div>
            </div>
            ...
            {!coupon ? (
                ...
                <p className="font-medium text-right">
                    ...
                    fallback={
                        `${currency}${
                        coupon
                            ? (totalPrice + 90 - ((coupon.discount || 0) / 100 * totalPrice)).toFixed(2)
                    ...
                    {`${currency}${
                        coupon
                        ? (totalPrice - ((coupon.discount || 0) / 100 * totalPrice)).toFixed(2)
